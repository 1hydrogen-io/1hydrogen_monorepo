...
export const ADDRESS = () =>
  isProduction()
    ? "0xf0d5ec2532c28e1fBF8493632FB710Da6B5c7f4A"
...
export default class VaultContract extends Erc20 {
  ...
  claimHsEthMutation = async (amount: number) => {
    const response: TransactionResponse = await this._contract.claimHsEth(
      this._toWei(amount),
      this._option
    );
  ...
  repayHsEthMutation = async (amount: number) => {
    const response: TransactionResponse = await this._contract.repayHsEth(
      this._toWei(amount),
      this._option
    );
    return this._handleTransactionResponse(response);
