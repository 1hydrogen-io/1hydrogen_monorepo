...
export const ADDRESS = () =>
  isProduction()
    ? "0xD90Bb8e51d5fEC92F95AD2d4Af6186BAA2ebBd75"
...
export default class UsdbVaultContract extends Erc20 {
  ...
  stakeMutation = async (amount: number) => {
    const rp: TransactionResponse = await this._contract.stake(
      this._toWei(amount),
      this._option
    );
    console.log(rp);
  ...
  claimHsUsdbMutation = async (amount: number) => {
    const response: TransactionResponse = await this._contract.claimHsUSDB(
      this._toWei(amount),
      this._option
    );
  ...
  repayHsUsdbMutation = async (amount: number) => {
    const response: TransactionResponse = await this._contract.repayHsUsdb(
      this._toWei(amount),
      this._option
    );
    return this._handleTransactionResponse(response);
