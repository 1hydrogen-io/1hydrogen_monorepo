...
export default function MintVaulContainer() {
  ...
  const onHandleAmountChange = (val: string) => {
    ...
    if (
      compareNumber(vaulStaked.availableBalance, amountNumber) &&
      isEthSelected
    )
      return;
    if (compareNumber(vaulStaked.stakedBalance, amountNumber) && isEthSelected)
      return;
    if (
      compareNumber(usdbVaulStaked.availableBalance, amountNumber) &&
      !isEthSelected
    )
      return;
    if (
      compareNumber(usdbVaulStaked.stakedBalance, amountNumber) &&
      !isEthSelected
    )
      return;
    setAmount(val);
  ...
  const onMint = async () => {
    try {
      ...
      if (!amountNumber) return onErrorToast("Invalid amount");
