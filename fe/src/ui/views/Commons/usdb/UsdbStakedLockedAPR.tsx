...
export default function UsdbStakedLockedAPR({isShow, isShowPoint}: IProps) {
    ...
    return (
        <>
            ...
            <Flex w="full" justifyContent="space-between">
                <LabelValueColumn
                    subLabel={"hsUSDB Balance"}
                    value={`${balance.hsEth} hsUSDB`}
                    label={`$${balance.hsEth * ethUsdt}`}
                    labelFontSize="12px"
                    subLabelFontSize="10px"
                />
                <LabelValueColumn
                    subLabel={"Staked hsUSDB Balance"}
                    value={`${stakedAmount} hsUSDB`}
                    label={`$${stakedAmount * ethUsdt}`}
                    labelFontSize="12px"
                    subLabelFontSize="10px"
                />
                <LabelValueColumn
                    subLabel={"Locked hsUSDB Balance"}
                    value={`${lockedHsETHBalance} hsUSDB`}
